        ...
        <div className="relative flex items-start justify-between border-b border-luxury-ink/8 px-6 py-5">
          <div className="flex items-center gap-3.5">
            <span
              ...
              style={{ borderRadius: "2px" }}
            >
              <IconAlertCircle className="h-5 w-5 text-luxury-champagne" strokeWidth={1.75} />
            </span>
            <div>
              <h2
              ...
              <p className="mt-0.5 text-xs text-neutral-500">Vui lòng cung cấp đầy đủ thông tin</p>
            </div>
          </div>
          <button
            type="button"
              ...
              <div key={label} className="flex flex-1 items-center last:flex-none">
                <div className="flex items-center gap-2">
                  <span
                    className={
                      isCurrent
                        ? "flex h-6 w-6 shrink-0 items-center justify-center bg-luxury-ink text-[11px] font-semibold text-luxury-champagne"
                        : isDone
                          ? "flex h-6 w-6 shrink-0 items-center justify-center bg-luxury-champagne text-luxury-ink"
                          : "flex h-6 w-6 shrink-0 items-center justify-center border border-luxury-ink/20 text-[11px] font-semibold text-taupe-400"
                    }
                    style={{ borderRadius: "2px" }}
                  >
                    {isDone ? <IconCheck className="h-3 w-3" strokeWidth={2.5} /> : stepNum}
                  </span>
                  <span
                    className={
                      isCurrent
                        ? "hidden text-[11px] font-semibold uppercase tracking-[0.12em] text-luxury-ink sm:inline"
            ...
            <div className="space-y-5 px-6 py-6">
              ...
              {step === 1 && (
                <div className="space-y-4">
                  <div>
                    <h4 style={{ fontFamily: "var(--font-droid-serif), serif" }} className="text-base text-luxury-ink">
                      Chọn lý do hoàn tiền
                    </h4>
                    <p className="mt-0.5 text-xs text-neutral-500">Chọn lý do phù hợp nhất với vấn đề của bạn</p>
                  </div>
                      ...
                      return (
                        ...
                        >
                          <div
                            ...
                            style={{ borderRadius: "2px" }}
                          >
                            <Icon className={isSelected ? "h-4.5 w-4.5 text-luxury-champagne" : "h-4.5 w-4.5 text-taupe-400"} strokeWidth={1.5} />
                          </div>
                          <div className="min-w-0 flex-1 pt-0.5">
                            <p className={isSelected ? "text-sm font-medium leading-tight text-luxury-ink" : "text-sm font-medium leading-tight text-neutral-700"}>
                              {opt.label}
                            </p>
                            <p className="mt-0.5 text-xs leading-snug text-neutral-400">{opt.desc}</p>
                          </div>
                          {isSelected && (
                            <span
                              ...
                              style={{ borderRadius: "2px" }}
                            >
                              <IconCheck className="h-2.5 w-2.5 text-luxury-ink" strokeWidth={3} />
                            </span>
                          )}
                        </button>
                      );
                  ...
                  <div className="flex gap-2.5 border border-luxury-ink/8 bg-white/60 p-3.5" style={{ borderRadius: "2px" }}>
                    <IconAlertCircle className="mt-0.5 h-4 w-4 shrink-0 text-taupe-400" strokeWidth={1.75} />
                    <p className="text-xs leading-relaxed text-neutral-600">
                      Yêu cầu sẽ được xem xét trong vòng <strong className="text-luxury-ink">24–48 giờ</strong>. Cung cấp
                      thông tin chính xác giúp xử lý nhanh hơn.
                    </p>
                  </div>
                </div>
              )}
              ...
              {step === 2 && (
                <div className="space-y-5">
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-neutral-500">Cung cấp chi tiết và hình ảnh để tăng tốc xử lý</p>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="mt-1.5 flex items-center justify-between">
                      <p className={description.trim().length < 10 && description.length > 0 ? "text-xs text-red-600/80" : "text-xs text-taupe-400"}>
                      ...
                      {description.trim().length >= 10 && (
                        <span className="flex items-center gap-1 text-xs font-medium text-luxury-ink">
                          <IconCheck className="h-3.5 w-3.5 text-luxury-champagne" strokeWidth={2.5} />
                          Đủ mô tả
                        </span>
                      )}
                    </div>
                  ...
                  <div>
                    <div className="mb-2 flex items-center justify-between">
                      ...
                      <span className="border border-luxury-ink/10 bg-cream-50 px-2 py-0.5 text-[10px] font-medium text-taupe-500" style={{ borderRadius: "2px" }}>
                    ...
                    >
                      {images.length === 0 ? (
                        <button
                          ...
                          style={{ borderRadius: "2px" }}
                        >
                          <span className="flex h-10 w-10 items-center justify-center border border-luxury-ink/12 bg-cream-50" style={{ borderRadius: "2px" }}>
                            <IconUpload className="h-4.5 w-4.5 text-taupe-400" strokeWidth={1.5} />
                          </span>
                          <p className="text-[13px] font-medium text-neutral-600">
                            {isDragging ? "Thả ảnh vào đây" : "Kéo & thả, hoặc click để chọn"}
                          </p>
                          <p className="text-[11px] text-taupe-400">JPG, PNG, WEBP — tối đa {REFUND_MAX_IMAGES} ảnh</p>
                        </button>
                      ) : (
                        <div className="p-3">
                          <div className="grid grid-cols-5 gap-2">
                            {images.map((file, i) => (
                              <div key={i} className="group relative aspect-square overflow-hidden border border-luxury-ink/10" style={{ borderRadius: "2px" }}>
                            ...
                            {images.length < REFUND_MAX_IMAGES && (
                              <button
                                ...
                                style={{ borderRadius: "2px" }}
                              >
                                <IconUpload className="h-4 w-4 text-taupe-400" strokeWidth={1.5} />
                              </button>
                            )}
                          </div>
                        </div>
                  ...
                  <div>
                    ...
                    {videos.length > 0 && (
                      <ul className="mb-2 space-y-2">
                        {videos.map((file, i) => (
                          <li key={i} className="flex items-center gap-3 border border-luxury-ink/10 bg-white/60 px-3 py-2.5" style={{ borderRadius: "2px" }}>
                            <span className="flex h-8 w-8 shrink-0 items-center justify-center bg-luxury-ink/5" style={{ borderRadius: "2px" }}>
                              <IconVideo className="h-4 w-4 text-taupe-500" strokeWidth={1.5} />
                            </span>
                            <div className="min-w-0 flex-1">
                              <p className="truncate text-sm font-medium text-luxury-ink">{file.name}</p>
                              <p className="text-xs text-taupe-400">{formatFileSize(file.size)}</p>
                            </div>
                            <button
                              type="button"
              ...
              {step === 3 && (
                <div className="space-y-4">
                  ...
                  <div className="flex items-start gap-3 border border-luxury-champagne/30 bg-luxury-champagne/8 p-4" style={{ borderRadius: "2px" }}>
                    <IconBuildingBank className="mt-0.5 h-4.5 w-4.5 shrink-0 text-taupe-700" strokeWidth={1.75} />
                    <p className="text-xs leading-relaxed text-neutral-700">
                      Thông tin ngân hàng được mã hóa và chỉ dùng để chuyển khoản hoàn tiền. Bạn sẽ không cần nhập lại
                      sau này.
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <IconBuildingBank className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-taupe-400" strokeWidth={1.75} />
                    </div>
                  </div>
              ...
              {step === 4 && (
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-neutral-500">Kiểm tra lại thông tin trước khi gửi</p>
                  </div>

                  {selectedReason && SelectedIcon && (
                    <div className="flex items-center gap-3 border border-luxury-champagne/40 bg-cream-50 p-4" style={{ borderRadius: "2px" }}>
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center bg-luxury-ink" style={{ borderRadius: "2px" }}>
                        <SelectedIcon className="h-5 w-5 text-luxury-champagne" strokeWidth={1.5} />
                      </div>
                      <div>
                        <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-taupe-400">Lý do</p>
                        <p className="text-sm font-medium text-luxury-ink">{selectedReason.label}</p>
                        <p className="mt-0.5 text-xs text-neutral-500">{selectedReason.desc}</p>
                      </div>
                    </div>
                  )}

                  <div className="border border-luxury-ink/8 bg-white/60 p-4" style={{ borderRadius: "2px" }}>
                    <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-taupe-400">Mô tả</p>
                    ...
                    <div className="border border-luxury-ink/8 bg-white/60 p-4" style={{ borderRadius: "2px" }}>
                      ...
                      <div className="space-y-0.5 text-sm">
                        <p>
                          ...
                          <span className="font-medium text-luxury-ink">{bankName}</span>
                        </p>
                        <p>
                          <span className="text-neutral-500">Số TK:</span>{" "}
                          <span className="font-mono font-medium text-luxury-ink">{accountNumber}</span>
                        </p>
                        <p>
                          <span className="text-neutral-500">Chủ TK:</span>{" "}
                          <span className="font-medium text-luxury-ink">{accountHolder}</span>
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="flex items-center gap-3 border border-luxury-ink/8 bg-white/60 p-4" style={{ borderRadius: "2px" }}>
                      <div className="flex h-9 w-9 shrink-0 items-center justify-center bg-luxury-ink/5" style={{ borderRadius: "2px" }}>
                        <IconPhoto className="h-4.5 w-4.5 text-taupe-500" strokeWidth={1.5} />
                      </div>
                      <div>
                        <p className="text-xs text-neutral-500">Ảnh bằng chứng</p>
                        <p className="text-xl font-semibold leading-none text-luxury-ink">
                          {images.length}
                          <span className="ml-1 text-sm font-normal text-taupe-400">ảnh</span>
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 border border-luxury-ink/8 bg-white/60 p-4" style={{ borderRadius: "2px" }}>
                      <div className="flex h-9 w-9 shrink-0 items-center justify-center bg-luxury-ink/5" style={{ borderRadius: "2px" }}>
                        <IconVideo className="h-4.5 w-4.5 text-taupe-500" strokeWidth={1.5} />
                      </div>
                      <div>
                        <p className="text-xs text-neutral-500">Video bằng chứng</p>
                        <p className="text-xl font-semibold leading-none text-luxury-ink">
                          {videos.length}
                          <span className="ml-1 text-sm font-normal text-taupe-400">video</span>
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  {images.length > 0 && (
                    <div className="grid grid-cols-5 gap-2">
                      {images.slice(0, 5).map((file, i) => (
                        <div key={i} className="aspect-square overflow-hidden border border-luxury-ink/10" style={{ borderRadius: "2px" }}>
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          <img src={URL.createObjectURL(file)} alt={`img-${i}`} className="h-full w-full object-cover" />
                        </div>
                      ))}
                      {images.length > 5 && (
                        <div className="flex aspect-square items-center justify-center border border-luxury-ink/10 bg-cream-50" style={{ borderRadius: "2px" }}>
                          <span className="text-sm font-semibold text-taupe-500">+{images.length - 5}</span>
                        </div>
                      )}
                    </div>
                  )}

                  <div className="flex gap-3 border border-luxury-ink/8 bg-white/60 p-4" style={{ borderRadius: "2px" }}>
                    <IconShield className="mt-0.5 h-5 w-5 shrink-0 text-luxury-champagne" strokeWidth={1.5} />
                    <div>
                      <p className="text-sm font-medium text-luxury-ink">Cam kết bảo vệ người mua</p>
                      <p className="mt-0.5 text-xs leading-relaxed text-neutral-600">
                        Yêu cầu được xem xét trong <strong className="text-luxury-ink">24–48 giờ</strong>. Nếu được chấp
                        thuận, tiền hoàn về trong <strong className="text-luxury-ink">3–7 ngày làm việc</strong>.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex items-center gap-3">
              {step > 1 ? (
                <button
                  ...
                  disabled={isSubmitting}
                  className="flex items-center gap-1.5 border border-luxury-ink/15 px-5 py-2.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-neutral-600 transition-all duration-300 hover:border-luxury-ink/30 hover:text-luxury-ink disabled:opacity-50"
                  style={{ borderRadius: "2px" }}
                >
                ...
                <button
                  ...
                  onClick={onClose}
                  disabled={isSubmitting}
                  className="border border-luxury-ink/15 px-5 py-2.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-neutral-600 transition-all duration-300 hover:border-luxury-ink/30 hover:text-luxury-ink disabled:opacity-50"
                  style={{ borderRadius: "2px" }}
                >
                  Hủy
                </button>
              ...
              {step < 4 ? (
                <button
                  ...
                  onClick={() => setStep((s) => s + 1)}
                  disabled={!canProceed}
                  className="group flex items-center gap-2 bg-luxury-ink px-6 py-2.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-luxury-ivory transition-all duration-300 hover:bg-charcoal-800 disabled:cursor-not-allowed disabled:opacity-30"
                  style={{ borderRadius: "2px" }}
                >
                  Tiếp theo
                  <IconArrowRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-0.5" strokeWidth={1.75} />
                </button>
              ) : (
                <button
                  type="button"
                  ...
                  disabled={isSubmitting}
                  className="group flex items-center gap-2 bg-luxury-ink px-6 py-2.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-luxury-ivory transition-all duration-300 hover:bg-charcoal-800 disabled:cursor-not-allowed disabled:opacity-50"
                  style={{ borderRadius: "2px" }}
                >
                  {isSubmitting ? (
                    <>
