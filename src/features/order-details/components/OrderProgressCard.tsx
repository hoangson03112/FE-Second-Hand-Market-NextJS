...
const TIPS_BY_STATUS: Record<string, string> = {
  ...
  confirmed:
    "Đã xác nhận. Nếu chưa thanh toán, chuyển khoản theo thông tin bên phải.",
  picked_up:
    "Đơn đã được GHN lấy hàng. Bạn có thể theo dõi vận đơn ở trên trang.",
  ...
  return (
    ...
    >
      ...
      <div className="px-5 py-4">
              ...
              <li key={step.key} className="flex items-start gap-3">
                <div className="flex shrink-0 flex-col items-center">
                  ...
                  >
                    {isDone || isActive ? (
                      ...
                      <IconCircle
                        className="h-3 w-3 text-taupe-300"
                        strokeWidth={2}
                      />
                    )}
                  </div>
                  {!isLast && (
                    <div
                      className={
                        i < effectiveStepIdx
                          ? "mt-1 min-h-[20px] w-px flex-shrink-0 bg-luxury-champagne/50"
                          : "mt-1 min-h-[20px] w-px flex-shrink-0 bg-luxury-ink/10"
                      }
                    />
                  )}
                </div>
                <div className={!isLast ? "pb-1" : ""}>
                  <p
                    className={
                      isDone || isActive
                        ? "text-sm font-medium text-luxury-ink"
                        : "text-sm text-taupe-400"
                    }
                  >
                    {step.shortLabel}
                  </p>
        ...
        <div
          className="mt-4 flex gap-2 border-t border-luxury-ink/8 bg-cream-50 p-3 pt-4"
          style={{ borderRadius: "2px" }}
        >
          <IconInfoCircle
