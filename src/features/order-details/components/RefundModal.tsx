...
export function RefundModal({
  ...
  const selectedReason = REASON_OPTIONS.find((r) => r.value === reason) ?? null;
  const SelectedIcon = selectedReason?.icon ?? null;
  const canProceed =
    step === 1 ? reason !== "" : step === 2 ? description.trim().length >= 10 : true;

  const handleImageInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = Array.from(e.target.files ?? []);
    onImagesChange([...images, ...files].slice(0, MAX_IMAGES));
    e.target.value = "";
  };

  const handleVideoInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = Array.from(e.target.files ?? []);
    onVideosChange([...videos, ...files].slice(0, MAX_VIDEOS));
    e.target.value = "";
  };
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (imgFiles.length) onImagesChange([...images, ...imgFiles].slice(0, MAX_IMAGES));
    if (vidFiles.length) onVideosChange([...videos, ...vidFiles].slice(0, MAX_VIDEOS));
  };
              ...
              {step === 1 && (
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-xs text-neutral-500 mt-0.5">Chọn lý do phù hợp nhất với vấn đề của bạn</p>
                  </div>
                    ...
                    {REASON_OPTIONS.map((opt) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            <Icon className={`w-4.5 h-4.5 ${isSelected ? "text-primary" : "text-neutral-500"}`} />
                          </div>
                  ...
                  <div className="flex gap-2.5 p-3 rounded-xl bg-neutral-100/80 border border-neutral-200">
                    ...
                    <p className="text-xs text-neutral-600 leading-relaxed">
                      Yêu cầu sẽ được xem xét trong vòng <strong className="text-neutral-800">24–48 giờ</strong>.
              ...
              {step === 2 && (
                <div className="space-y-5">
                  <div>
                    ...
                    <p className="text-xs text-neutral-500 mt-0.5">Cung cấp chi tiết và hình ảnh để tăng tốc xử lý</p>
                  </div>

                  {/* Textarea */}
                  <div>
                    <label className="block text-xs font-semibold text-neutral-700 mb-1.5 uppercase tracking-wide">
                      Mô tả chi tiết <span className="text-destructive normal-case tracking-normal">*</span>
                    </label>
                    <textarea
                      value={description}
                    ...
                    <div className="flex items-center justify-between mt-1.5">
                      <p className={`text-xs ${description.trim().length < 10 && description.length > 0 ? "text-destructive/70" : "text-neutral-400"}`}>
                      ...
                      {images.length === 0 ? (
                        ...
                        <div className="p-3">
                          <div className="grid grid-cols-5 gap-2">
                            {images.map((file, i) => (
                              <div key={i} className="relative group aspect-square rounded-xl overflow-hidden border border-neutral-200/80">
                  ...
                  <div>
                    ...
                    {videos.length > 0 && (
                      <ul className="space-y-2 mb-2">
                        {videos.map((file, i) => (
                          <li key={i} className="flex items-center gap-3 px-3 py-2.5 bg-neutral-100/70 border border-neutral-200/80 rounded-xl">
                    ...
                    {videos.length < MAX_VIDEOS && (
                      <>
                        ...
                        >
                          ...
                          Thêm video{videos.length > 0 ? ` (còn ${MAX_VIDEOS - videos.length} slot)` : ""}
                        </button>
                      </>
                    )}
                  </div>
              ...
              {step === 3 && (
                <div className="space-y-4">
                  ...
                  <div className="p-4 rounded-2xl bg-primary/5 border border-primary/15 flex items-start gap-3">
                    ...
                    <p className="text-xs text-neutral-600 leading-relaxed">
                      Thông tin ngân hàng được mã hóa và chỉ dùng để chuyển khoản hoàn tiền.
                      Bạn sẽ không cần nhập lại sau này.
                    </p>
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 rounded-xl border border-neutral-200 bg-white text-sm focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all disabled:opacity-50"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-neutral-700 uppercase tracking-wide mb-1.5">
                ...
                <div className="space-y-4">
                  ...
                  {selectedReason && SelectedIcon && (
                    <div className="flex items-center gap-3 p-4 rounded-2xl bg-primary/8 border-2 border-primary/25">
                      ...
                      <div>
                        <p className="text-[10px] text-neutral-500 font-semibold uppercase tracking-widest">Lý do</p>
                  ...
                  <div className="p-4 rounded-2xl bg-neutral-100/70 border border-neutral-200/80">
                    <p className="text-[10px] text-neutral-500 font-semibold uppercase tracking-widest mb-2">Mô tả</p>
                  ...
                  {bankName && accountNumber && accountHolder && (
                    <div className="p-4 rounded-2xl bg-neutral-100/70 border border-neutral-200/80">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="text-[10px] text-neutral-500 font-semibold uppercase tracking-widest">Ngân hàng nhận hoàn</p>
                      </div>
                      <div className="space-y-0.5 text-sm">
                        <p><span className="text-neutral-500">Ngân hàng:</span> <span className="font-semibold">{bankName}</span></p>
                        <p><span className="text-neutral-500">Số TK:</span> <span className="font-mono font-semibold">{accountNumber}</span></p>
                        <p><span className="text-neutral-500">Chủ TK:</span> <span className="font-semibold">{accountHolder}</span></p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex gap-3 p-4 rounded-2xl bg-neutral-100/70 border border-neutral-200/80">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-neutral-600 mt-0.5 leading-relaxed">
                        Yêu cầu được xem xét trong <strong>24–48 giờ</strong>. Nếu được chấp thuận,
                        tiền hoàn về trong <strong>3–7 ngày làm việc</strong>.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {STEPS.map((_, i) => (
                  <div
                    ...
                    className={`rounded-full transition-all ${
                      step === i + 1 ? "w-4 h-2 bg-neutral-800" : i + 1 < step ? "w-2 h-2 bg-primary" : "w-2 h-2 bg-neutral-300"
                    }`}
                  />
                ))}
              </div>
