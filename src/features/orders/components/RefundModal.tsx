                ...
                <div className="space-y-5">
                  ...
                  <div>
                    ...
                    <div className="flex items-center justify-between mt-1.5">
                      <p className={`text-xs ${refundDescription.trim().length < 10 && refundDescription.length > 0 ? "text-destructive/70" : "text-neutral-400"}`}>
                        {refundDescription.trim().length < 10 && refundDescription.length > 0
                          ? `Cần thêm ${10 - refundDescription.trim().length} ký tự`
                          ...
                          <div className="grid grid-cols-5 gap-2">
                            {refundImages.map((file, i) => (
                              <div key={i} className="relative group aspect-square rounded-xl overflow-hidden border border-neutral-200/80">
                                ...
                                <button
                                  type="button"
                                  onClick={() => onImagesChange(refundImages.filter((_, idx) => idx !== i))}
                                  disabled={isSubmitting}
                      ...
                      <ul className="space-y-2 mb-2">
                        {refundVideos.map((file, i) => (
                          <li key={i} className="flex items-center gap-3 px-3 py-2.5 bg-neutral-100/70 border border-neutral-200/80 rounded-xl">
                            ...
                            <button
                              type="button"
                              onClick={() => onVideosChange(refundVideos.filter((_, idx) => idx !== i))}
                              disabled={isSubmitting}
              ...
              {step === 3 && (
                <div className="space-y-4">
                  ...
                  <div className="p-4 rounded-2xl bg-neutral-100/70 border border-neutral-200/80">
                    ...
                    <p className="text-sm text-neutral-800 leading-relaxed">{refundDescription}</p>
                  </div>
                  ...
                  {refundImages.length > 0 && (
                    <div className="grid grid-cols-5 gap-2">
                      {refundImages.slice(0, 5).map((file, i) => (
                      ...
                      {refundImages.length > 5 && (
                        <div className="aspect-square rounded-xl bg-neutral-100 border border-neutral-200/80 flex items-center justify-center">
                          <span className="text-sm font-bold text-neutral-500">+{refundImages.length - 5}</span>
                        </div>
                      )}
                    </div>
                  )}
