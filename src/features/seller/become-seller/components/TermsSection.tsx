    ...
    <div>
      <label className="group flex cursor-pointer items-start gap-3 py-1">
        ...
        <span
          className={
            checked
              ? "rounded-[2px] mt-0.5 flex h-[18px] w-[18px] shrink-0 items-center justify-center border border-luxury-ink bg-luxury-ink transition-all duration-200"
              : "rounded-[2px] mt-0.5 flex h-[18px] w-[18px] shrink-0 items-center justify-center border border-luxury-ink/25 bg-white transition-all duration-200 group-hover:border-luxury-ink/50"
          }
        >
      ...
      {error && <p className="ml-8 mt-1 text-xs text-blush-600">{error}</p>}
    </div>
  );
}
...
export const TermsSection: React.FC<TermsSectionProps> = ({ values, errors, onChange }) => {
  return (
    <div className="border border-luxury-ink/8 bg-white/50 p-5 md:p-6 rounded-[2px]" >
