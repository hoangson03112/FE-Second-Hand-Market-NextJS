...
export default function ProductActionButtons({
  ...
  if (isOutOfStock) {
    return (
      <div className="w-full py-4 rounded-[2px] bg-taupe-50 text-taupe-400 border border-luxury-ink/10 font-bold text-xs uppercase tracking-[0.15em] text-center">
        Hết hàng
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-row gap-4">
      ...
      <button
        onClick={onAddToCart}
        disabled={disabled}
        className="flex-1 flex items-center justify-center gap-2 py-4 rounded-[2px] bg-white border border-luxury-ink/20 text-luxury-ink font-bold text-xs uppercase tracking-[0.15em] hover:bg-taupe-50 hover:border-luxury-ink/40 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
      >
      ...
      <button
        onClick={onBuyNow}
        disabled={disabled}
        className="flex-1 flex items-center justify-center gap-2 py-4 rounded-[2px] bg-luxury-ink text-white font-bold text-xs uppercase tracking-[0.15em] disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 hover:bg-luxury-ink/90"
      >
