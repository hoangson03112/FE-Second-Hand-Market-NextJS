        ...
        >
          <div className="flex gap-4 p-4">
            ...
            <div className="flex-1 min-w-0">
              <h4 className="font-semibold text-foreground line-clamp-2 mb-2">{item.name}</h4>
              {item.condition && (
          ...
          {item.seller && (
            <div className="px-4 py-3 bg-muted/30 border-t border-border">
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  {item.seller.avatar && (
                    <img src={item.seller.avatar} alt={item.seller.name} className="w-5 h-5 rounded-full" />
                  )}
                  <span className="text-sm font-medium text-foreground">{item.seller.name}</span>
                </div>
              </div>
            </div>
          )}
