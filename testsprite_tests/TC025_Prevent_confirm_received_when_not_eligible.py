...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Đăng nhập' link to open the login page (index 89).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/header/div/div/div[2]/div/div/a').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Fill the username and password fields and submit the login form (perform login). After that, locate and open the Orders page to find an order that is not receivable and verify there is no confirm-receipt action.
        frame = context.pages[-1]
        # Input text
        ...
        frame = context.pages[-1]
        assert await frame.locator("xpath=//*[contains(., 'Không có hành động nào')]").nth(0).is_visible(), "The order page should indicate there are no available actions to confirm receipt for this order"
