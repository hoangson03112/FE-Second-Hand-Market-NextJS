...
async def run_test():
    ...
    try:
        ...
        # -> Fill the username and password fields and submit the login form to sign in as 'viet1'.
        frame = context.pages[-1]
        # Input text
        ...
        # -> Attempt to sign in again by submitting the login form, then wait for the UI to update.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/main/div/div[2]/div/div/div/div[2]/div[2]/form/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        ...
        frame = context.pages[-1]
        assert await frame.locator("xpath=//*[contains(., 'Pending')]").nth(0).is_visible(), "The orders list should display at least one order row with a status after signing in"
