...
async def run_test():
    ...
    try:
        ...
        # -> Fill the username and password fields and submit the login form (input username into index 551, password into index 563, then click submit at index 591).
        frame = context.pages[-1]
        # Input text
        ...
        # -> Retry submitting the login form to sign in (click the 'Đăng nhập' submit button).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/main/div/div[2]/div/div/div/div[2]/div[2]/form/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        ...
        frame = context.pages[-1]
        assert await frame.locator("xpath=//*[contains(., 'Tổng cộng')]").nth(0).is_visible(), "The cart should show the updated total after changing the item quantity"
