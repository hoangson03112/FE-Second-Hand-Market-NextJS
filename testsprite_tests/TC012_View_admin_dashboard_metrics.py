...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Đăng nhập' (Login) link to open the login page (element index 114).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/header/div/div/div[2]/div/div/a').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Fill the username field with the admin username (viet1).
        frame = context.pages[-1]
        # Input text
        ...
        # -> Retry submitting the login form by clicking the 'Đăng nhập' submit button, then wait for the UI to settle and check whether the admin dashboard or any dashboard links/metrics appear.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/main/div/div[2]/div/div/div/div[2]/div[2]/form/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        ...
        frame = context.pages[-1]
        assert await frame.locator("xpath=//*[contains(., 'Tổng quan')]").nth(0).is_visible(), "The dashboard should show an operational summary section after successful admin login"
