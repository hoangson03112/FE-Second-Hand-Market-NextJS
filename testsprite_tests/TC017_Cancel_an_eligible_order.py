...
async def run_test():
    ...
    try:
        ...
        # -> Retry submitting the login form to attempt sign-in again (click the 'Đăng nhập' submit button).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/main/div/div[2]/div/div/div/div[2]/div[2]/form/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the 'Đăng nhập' button to retry signing in (attempt 3), then wait for the page to settle and locate the Orders page.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/main/div/div[2]/div/div/div/div[2]/div[2]/form/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        ...
        frame = context.pages[-1]
        assert await frame.locator("xpath=//*[contains(., 'Đã hủy')]").nth(0).is_visible(), "The orders list should show Đã hủy after cancelling the order"
