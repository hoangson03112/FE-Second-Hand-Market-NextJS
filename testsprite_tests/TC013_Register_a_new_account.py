...
async def run_test():
    ...
    try:
        ...
        # -> Fill the registration form fields (Họ và tên, Tên đăng nhập, Email, Số điện thoại, Mật khẩu, Xác nhận mật khẩu) and submit the form, then verify a confirmation message that the account was created is visible.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/main/div/div[2]/div/div/div/div[2]/div[2]/form/div/div/div/input').nth(0)
        await asyncio.sleep(3); await elem.fill('Viet Nguyen')
        ...
        await asyncio.sleep(3); await elem.fill('viet1.test@example.com')
        
        # -> Fill the phone, password, and confirm password fields, then submit the registration form by clicking the 'Đăng ký' submit button.
        frame = context.pages[-1]
        # Input text
        ...
        # -> Click the 'Đăng ký' submit button (index 613), wait for the page to update, then check the page for a visible account-creation confirmation message.
        frame = context.pages[-1]
        # Click element
        ...
        frame = context.pages[-1]
        assert await frame.locator("xpath=//*[contains(., 'Đăng ký thành công')]").nth(0).is_visible(), "The page should display Đăng ký thành công after submitting the registration form."
