...
async def run_test():
    ...
    try:
        ...
        # -> Wait for the login processing to finish, then open the product catalog (Danh mục) to select a product.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/header/div/div/a').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Open the login page by clicking the 'Đăng nhập' link so I can re-submit credentials.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/header/div/div/div[2]/div/div/a').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Fill the username and password fields and submit the login form (attempt 2).
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/main/div/div[2]/div/div/div/div[2]/div[2]/form/div/div/div/input').nth(0)
        await asyncio.sleep(3); await elem.fill('viet1')
        ...
        frame = context.pages[-1]
        assert await frame.locator("xpath=//*[contains(., 'Giỏ hàng')]").nth(0).is_visible(), "The cart should display at least one item after adding a product."
