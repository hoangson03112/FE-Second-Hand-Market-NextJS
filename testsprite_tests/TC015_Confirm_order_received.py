...
async def run_test():
    ...
    try:
        ...
        # -> Retry submitting the login form to attempt sign-in again by clicking the 'Đăng nhập' button.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/main/div/div[2]/div/div/div/div[2]/div[2]/form/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        ...
        frame = context.pages[-1]
        assert await frame.locator("xpath=//*[contains(., 'Đã nhận')]").nth(0).is_visible(), "The order should show a Đã nhận status and the timeline should reflect the received update after confirming receipt."
