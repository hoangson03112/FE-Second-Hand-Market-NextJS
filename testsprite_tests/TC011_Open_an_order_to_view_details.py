...
async def run_test():
    ...
    try:
        ...
        # -> Fill the username field (index 545) with 'viet1', fill the password field (index 557) with '12345678nM', then submit the login form by clicking the 'Đăng nhập' button (index 585).
        frame = context.pages[-1]
        # Input text
        ...
        # -> Retry submitting the login form to attempt sign-in again by clicking the 'Đăng nhập' submit button, then wait for the UI to update.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/main/div/div[2]/div/div/div/div[2]/div[2]/form/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        ...
        frame = context.pages[-1]
        assert await frame.locator("xpath=//*[contains(., 'Chi tiết đơn hàng')]").nth(0).is_visible(), "The order details view should display 'Chi tiết đơn hàng' after opening an order from the orders list"
